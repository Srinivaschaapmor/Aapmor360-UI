...
function JobCardPublicView({ job }) {
  const {
    _id,
    position,
    about,
  ...
  return (
    ...
    >
      <Stack onClick={() => navigate(`/careers/job-openings/${_id}`)}>
        ...
        <Stack direction={"row"} width="90%" gap={1} my={0.5}>
          ...
          <Stack
            direction={"row"}
            width={experience === "entryLevel" ? "12%" : "10%"}
            alignItems={"center"}
        ...
        >
          ...
          >
            {/* <Button
              onClick={() => navigate(`/job-openings/${_id}#applicants`)}
              // onClick={()=> navigation.push(`/job-openings/${_id}`, { hashtags: 'applicants' })}
              sx={{
...
export default JobCardPublicView;
